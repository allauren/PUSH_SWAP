...
/*   Updated: 2017/12/13 14:14:04 by allauren         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_easy_sort(t_2pile *pile, int i)
{
	...
	else if(ft_2listtail(pile->pileb)->value < ft_2listhead(pile->pileb)->value && (ft_set_values("rrb", 0)))
		pile->pileb = ft_2listrevrotate(pile->pileb);
	else if(ft_2listtail(pile->pilea)->value < ft_2listhead(pile->pilea)->value && (ft_set_values("rra", 0)))
		pile->pilea = ft_2listrevrotate(pile->pilea);
	ft_print_pile(pile);
	}
